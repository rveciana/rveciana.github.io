---
layout: ../layouts/Page.astro
title: About
---

import config from "../config.json";

<img
  src="http://www.gravatar.com/avatar/a2746d9ceb326e71181a6629031faf81?s=135"
  alt={`A photo of ${config.author.name}`}
  class="left_picture"
/>

<h2>{config.author.name}</h2>
I'm a meteorologist based in Barcelona who likes maps and coding. [You can take a
look at my CV](/cv)

I opened this blog to talk about the things I'm learning about python and JavaScript mapping.

[The old GeoExamples blog](http://geoexamples.blogspot.com.es/) is still working with all its posts.
