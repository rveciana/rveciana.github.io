...
/**
Manages all the data. Reads from the configuration file to set the initial active layers.
Has functions to add and remove active layers.
Reads the data formats and attaches the read data info the active data objects
*/
function dataManager(config, iniCallback){
  this.activeDataLayers = {};
  this.activeBackgroundLayers = {};
  this.layersQueue = queue();
  ...
  Object.keys(config.dataLayers).map(function(layerName){
    ...
    if(d.active){
      that.layersQueue.defer(activateGeotiffLayer, d, that.activeDataLayers, layerName);
    }
  });
  ...
  Object.keys(config.backgroundLayers).map(function(layerName){
    ...
    if(d.active){
      that.layersQueue.defer(activateTopojsonLayer, d, that.activeBackgroundLayers, layerName);
    }

  });


  this.layersQueue.await(function(error) {
...
dataManager.prototype.addDataLayer = function(layerConfig, layerName, callback){
     var that = this;
     Object.keys(this.activeDataLayers).forEach(function(activeLayerName){
       if(that.activeDataLayers[activeLayerName].type === layerConfig.type){
         delete that.activeDataLayers[activeLayerName];
       }
     });
    activateGeotiffLayer(layerConfig, this.activeDataLayers, layerName, callback);
};
...
function activateTopojsonLayer(layerConfig, activeBackgroundLayers, layerName, callback){
  activeBackgroundLayers[layerName] = {'type': 'topojson'};
...
function activateGeotiffLayer(layerConfig, activeDataLayers, layerName, callback){
  ...
  activeDataLayers[layerName]['type'] = layerConfig.type;
  request(layerConfig.file)
  ...
  .get(function(error, tiffData){
    ...
    activeDataLayers[layerName]['scale'] = layerConfig.scale;

    callback();
  });
}

function drawData(dataLayer, mapContext, path){
  if(dataLayer.type === "isobands"){
    drawIsobands(dataLayer, mapContext, path);
  } else if(dataLayer.type === "isolines"){
    drawIsolines(dataLayer, mapContext, path);
  }
}

function drawIsolines(dataLayer, mapContext, path){
  if(!dataLayer.isolines){
    dataLayer.isolines = isolines(dataLayer.data,
                dataLayer.geoTransform,
                dataLayer.scale);
  }
  console.info(dataLayer.isolines.features);
  dataLayer.isolines.features.forEach(function(d) {
...
function drawIsobands(dataLayer, mapContext, path){
  if(!dataLayer.bands){
    dataLayer.bands = isobands(dataLayer.data,
                dataLayer.geoTransform,
                dataLayer.scale.map(function(d){return d.value;})); //Posar scale a activeDataLayers
  }
  dataLayer.bands.features.forEach(function(d, i) {
    mapContext.beginPath();
    mapContext.globalAlpha = 0.7; //TODO: Configurable!
    mapContext.fillStyle = dataLayer.scale[i].color;
    path(d);
    mapContext.fill();
  });
}
...
function viewer(rootDiv, configFile, callback) {
  ...
  json(configFile, function(error, config) {
    ...
    var toolTip = rootDiv.append("div")
      ...
      .style("z-index", 10)
      .style("height", "auto") //Avoid calculating height
      .style("transition", ".2s") //Avoid movement transitions
      .style("opacity", 0);
    ...
    mapCanvas.on("click", function() {
      ...
      Object.keys(activeDataLayers).forEach(function(layerName){
        ...
        var value = activeDataLayers[layerName].data[Math.round(yTiff)][Math.round(xTiff)];
        toolTip
          .style("left", screenCoords[0] + "px")
          .style("top", screenCoords[1] + "px");
        toolTipText.html(activeDataLayers[layerName].model + "-" + activeDataLayers[layerName].variable +
   ...
   that.draw = function(){
      ...
      Object.keys(activeDataLayers).forEach(function(varName){
        drawData(activeDataLayers[varName], mapContext, path);
      });
...
viewer.prototype.toggleDataLayer = function(layerName){
  if(!(layerName in this.dataManager.getActiveDataLayers())) {
    this.dataManager.addDataLayer(this.config.dataLayers[layerName], layerName, this.draw);
  } else {
